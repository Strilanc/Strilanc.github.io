...
But... I would be exagerating if I said Quirk has reached a significant number of the people who could actually make use of it.
...
Dave Bacon refered to Quirk as "the most beautiful quantum circuit simulator ever".
...
Addition, multiplication, comparison, modular exponentation, and various other goodies that are tedious to build out of basic gates are now included out of the box.
...
For example, the shader for the increment and decrement gates used to be 20 finnicky lines long; now it's nearly a one-liner:
...
There used to be dozens and dozens of lines of boilerplate around gate tests, carefully setting up input pixel data to feed into a shader, circuits defined by giant transposed arrays of long gate names, ecclectic comparisons of the results against redundant computation performed using matrices, and on and on.
