...
I was stuck on this problem for awhile, but eventually realized that the trick was to mix in the *bitwise complement*.
...
- We will toggle the ancilla wire, and the high bits, before and after the decrement gate, whenever the top wires are all `On`.
    ...
    Either the borrowed ancilla bit will `Off`, meaning neither the increment nor the decrement gate will take effect, or the borrowed ancilla bit will be `On` and both the increment and the decrement gate will apply, undoing each other.
...
The single bit constructions we've covered let us turn any $n$-bit increment gate into two or three $n/2$-bit increments, depending on what type of ancilla bit we have.
...
The zeroed bits case is solved, as with the single bit case, by uncomputing. We simply sweep up after ourselves:

<img src="http://i.imgur.com/xs1NtN1.png" alt="Incrementer from n-2 Zeroed bits" height="300px"/>

That leaves only the $n$ borrowed bit case.

I have to admit, this case gave me a hard time.
I was stuck on it for at least a solid week, unable to come up with anything that worked.
...
It's difficult and specific enough that you'll get few responses on forums or Q/A sites, yet simple enough that it's not going to be the title of a paper (making it hard to find pre-existing solutions).
...
With the above circuit, we can subtract a garbage carry bit $c$ and a garbage two's-complement value $g$ out of our target value-to-increment $v$.

On its own, that subtraction seems like the wrong move to make because it mixes a bunch of garbage into our target.
However, we can use the bitwise complement trick from before to fix the problem.
If we apply the subtraction widget again, but toggle $g$'s bits first, then target bits will have transitioned from storing $v$ to storing $v-c-g$ to storing $v-c-g-(-g-1)-c = v-2c+1$.
...
Handling those is just a matter of applying the construction from last time.
